...
struct Person_Behaviour;

struct RPGHero_Behaviour;
...
typedef void (*Fn_DoAction)(RPGHero* self, JB_String* Reason);
...
typedef void (*SayerFunction)(JB_String* input);
...
struct Person_Behaviour: Object_Behaviour {
};

JBClass ( Person , JB_Object , 
	int age;
	JB_String* name;
);
...
struct RPGHero_Behaviour: Object_Behaviour {
};

JBClass ( RPGHero , JB_Object , 
	JB_String* Name;
	bool OnGround;
	bool IsDucking;
	int YSpeed;
...
extern Dictionary* JB_DoAction;
#define kJB_SaverEnd (JB_LUB[0])
#define kJB_SaverStart1 (JB_LUB[350])
