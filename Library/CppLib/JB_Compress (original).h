...
static bool arr_reserve(FastBuff& B,  JB_String* self,  int TotalLength,  int PrevLength,  FastString* fs) {
	auto Addr	= self->Addr;
	...
	B.Write = JB_FS_NeedSpareNoFlush(fs, B.Expected);
	DReq (B.Write, "Out of memory", 0);
	B.BackStart = fs->ResultPtr;
