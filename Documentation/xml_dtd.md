...
* DTDs give are bad user interfaces with a user-filled webform. Relying on DTD to check entries, the entire document is either valid or not. It makes for a bad user experience. The user wants friendly and helpful feedback on multiple fields. You don't get that with DTDs.
