...
I just call them 'isms'. The more 'isms' you know, the better your code will look. But you don't **need** to know them. In fact, its _harmful_ to think you **need** to know them.

You only really need to know 'isms' for **reading** code, not writing. Its no point looking at code you can't understand.
...
+ `syntax cast` - Decides how an object gets turned into a bool. Useful for `"if"` tests.
...
You'll also have to have linked `/usr/local/bin/spd` to your speedie executable, but this exact-path is the standard I've settled on. 

(The first run will take a few seconds to compile. Its cached after that.)

#### 'Is' and 'isnt'
Sometimes its nice to have constants actually work for you, rather than you work for them. Syntax "is" does this.
